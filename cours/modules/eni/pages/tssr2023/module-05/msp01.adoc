...
* [x] 3.3.2 Afin d’en tester l’accès, votre binôme génèrera un lecteur réseau « U: » qui pointera vers le dossier partagé. Ce lecteur devra apparaître automatiquement à l’ouverture de session. À cette fin, deux méthodes opérationnelles différentes devront être trouvées.

* [x] 3.3.3 Enfin, votre binôme en validera l’accès en écriture en y créant un fichier.

* -> xref:tssr2023/module-05/windows.adoc#3-2-occupation-des-espaces-disques[Procédure: Gestion Partages (Windows)]
