...
.options
****
* `-r/R`                       -> Suppression récursive. Appliquée à un répertoire, supprime le répertoire et son contenu, y compris tous les répertoires de niveau inférieur. 
* `-i`                         -> Demande de message de confirmation de suppression pour chaque fichier 
* `-f`                         -> Supprime le message de confirmation automatique si l’utilisateur (propriétaire) n'a pas le droit d'écriture sur le fichier 
* `--`                         -> Fin des options ou aucune option, utilisée si l'on souhaite supprimer un fichier dont le nom est –f ou –r par exemple. 
* `<fichier(s) répertoire(s)>` -> Nom du fichier ou répertoire à supprimer. Ce nom peut être générique.
****
...
|===
^.^| Syntaxe : `*rm* [options] <source(s)> <cible>  `
|=== 
